...
{
    cepVal::warp=posPeak[findNearestValueArray(posPeak)]/cepVal::winnerMoins1[0]*cepVal::warp;
...
double findNearestValueArray(vector <double> &grille)
{
    ...
    double smallest = abs(grille[0]-cepVal::winnerMoins1[0]) ; //declare and initiate smallest value at first case 0

    int position=0;// position of the smallest value
    for ( int i=1;  i < tailleArray;  ++i )
    {
        if ( abs(grille[i]-cepVal::winnerMoins1[0])  < smallest )// if case value smaller replace it
        {
            smallest=abs(grille[i]-cepVal::winnerMoins1[0]);
            position = i;
        }
    }
    return position;
